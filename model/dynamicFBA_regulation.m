function [concentrationMatrix,excRxnNames,timeVec,biomassVec] = ...
    dynamicFBA_regulation(model,substrateRxns,initConcentrations,initBiomass,timeStep,nSteps,plotRxns,exclUptakeRxns)
%dynamicFBA Perform dynamic FBA simulation using the static optimization
%approach
% HdJ (31/3/15): Hack to implement regulatory FBA
%
...
for stepNo = 1:nSteps
    % Run FBA
    model = changeRxnBounds(model,'Import_S2',regulationUptakeX2(concentrations(1)),'u');
...
legend(strrep(excRxnNames(selPlot),'EX_',''));
    ...
    function boundX2 = regulationUptakeX2(concX1)
